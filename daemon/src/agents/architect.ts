...
export async function runArchitectAgent(projectPath: string): Promise<void> {
  ...
  try {
    ...
    const files = db.prepare(`
      ...
      ORDER BY f.path
      LIMIT 200
    `).all() as Array<{ path: string; file_type: string; description: string | null; module_name: string | null }>;
...
Antworte NUR mit diesem JSON (kein Markdown, kein Text davor/danach):
{
  ...
  "summary": "2-3 Saetze Gesamtueberblick"
    ...
    let analysis: {
      ...
      critical_files?: Array<{ path: string; reason: string }>;
      summary?: string;
    };
