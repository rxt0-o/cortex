...
export async function runMoodScorerAgent(projectPath: string, sessionId: string, transcriptPath?: string): Promise<void> {
  ...
  try {
    // Get session summary
    const session = db.prepare(`SELECT summary FROM sessions WHERE id=?`).get(sessionId) as any;
