...
function updateFileCluster(db, recentSessions) {
...
export async function runPatternAgent(projectPath, sessionId) {
    ...
    `).all();
        ...
        const patternCount = db.prepare('SELECT COUNT(*) as c FROM work_patterns').get()?.c ?? 0;
...
async function predictIntent(db, projectPath, recentSessions) {
    let branch = 'unknown';
    try {
