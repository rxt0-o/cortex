...
export async function runCleanupAgent(projectPath) {
    ...
    try {
        ...
        const total = db.prepare(`SELECT COUNT(*) as c FROM sessions WHERE status='completed'`).get()?.c ?? 0;
        if (total === 0 || total % 5 !== 0)
            return;
        // Check if we already cleaned up at this count
        let lastCount = 0;
        try {
            const meta = db.prepare(`SELECT value FROM meta WHERE key='cleanup_last_count'`).get();
    ...
    `).all();
        if (learnings.length < 2 && staleDecisions.length === 0)
            return;
        const learningStr = learnings.map(l => `[ID:${l.id}] (${l.severity || 'medium'}, occ:${l.occurrences || 0}) ${l.anti_pattern} → ${l.correct_pattern}${l.context ? ' | ctx: ' + l.context : ''}`).join('\n');
        const decisionStr = staleDecisions.map(d => `[ID:${d.id}] (${d.category}) ${d.title}: ${d.reasoning}`).join('\n');
...
If no duplicates or stale decisions found, return empty arrays.`;
        ...
        if (Array.isArray(parsed.duplicate_pairs)) {
            for (const pair of parsed.duplicate_pairs) {
                if (typeof pair.keep_id !== 'number' || typeof pair.remove_id !== 'number')
                    continue;
                if (pair.keep_id === pair.remove_id)
                    continue;
                try {
                    db.prepare(`UPDATE learnings SET superseded_by=?, superseded_at=datetime('now') WHERE id=? AND superseded_by IS NULL`).run(pair.keep_id, pair.remove_id);
                    dupCount++;
                }
                catch (e) {
