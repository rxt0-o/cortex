      ...
      "relevance": "maybe_relevant"
        ...
        if (analysis.learnings) {
          ...
          `).get(l.anti_pattern);
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
          `).run(ts, l.anti_pattern, l.correct_pattern, l.context, l.severity ?? 'medium', l.auto_block ? 1 : 0, l.detection_regex ?? null, relevance, l.write_gate_reason ?? null);
        ...
        if (analysis.insights) {
            for (const i of analysis.insights) {
                if (!i.observation || !i.implication)
                    continue;
                try {
                    db.prepare(`
