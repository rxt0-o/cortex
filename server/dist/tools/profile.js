...
export function registerProfileTools(server) {
    ...
    server.tool('cortex_list_notes', 'List notes, optionally filtered by search term', {
        ...
        if (entity_type && entity_id && search) {
            notes = db.prepare(`SELECT * FROM notes WHERE entity_type=? AND entity_id=? AND text LIKE ? ORDER BY created_at DESC LIMIT ?`).all(entity_type, entity_id, `%${search}%`, limit);
        }
        else if (entity_type && entity_id) {
            notes = db.prepare(`SELECT * FROM notes WHERE entity_type=? AND entity_id=? ORDER BY created_at DESC LIMIT ?`).all(entity_type, entity_id, limit);
        }
        else if (search) {
            notes = db.prepare(`SELECT * FROM notes WHERE text LIKE ? ORDER BY created_at DESC LIMIT ?`).all(`%${search}%`, limit);
        }
        else {
            notes = db.prepare(`SELECT * FROM notes ORDER BY created_at DESC LIMIT ?`).all(limit);
        }
