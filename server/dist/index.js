...
const server = new McpServer({ name: 'project-cortex', version: '0.1.0' }, { instructions: CORTEX_INSTRUCTIONS });
// ═══════════════════════════════════════════════════
// SESSION TOOLS
// ═══════════════════════════════════════════════════
...
server.tool('cortex_list_errors', 'List known errors, optionally filtered by severity or file', {
...
// PROJECT MAP
// ═══════════════════════════════════════════════════
server.tool('cortex_get_map', 'Get project architecture map — modules, layers, files', {
...
server.tool('cortex_update_map', 'Re-scan the project and update the architecture map', { root_path: z.string().optional() }, async ({ root_path }) => {
    getDb();
    const result = projectMap.scanProject(root_path ?? process.cwd());
    return { content: [{ type: 'text', text: JSON.stringify({ success: true, ...result }, null, 2) }] };
});
server.tool('cortex_scan_project', 'Scan project filesystem and populate architecture map with all files, modules and dependencies', { root_path: z.string().optional() }, async ({ root_path }) => {
    getDb();
    const result = projectMap.scanProject(root_path ?? process.cwd());
    return { content: [{ type: 'text', text: JSON.stringify({ success: true, ...result }, null, 2) }] };
});
...
// INDEX DOCS
// ═══════════════════════════════════════════════════
server.tool('cortex_index_docs', 'Read CLAUDE.md and docs/ markdown files and store as searchable learnings and decisions', { docs_path: z.string().optional() }, async ({ docs_path }) => {
    getDb();
...
// HOT ZONES
// ═══════════════════════════════════════════════════
server.tool('cortex_get_hot_zones', 'Get most frequently changed files — refactoring candidates', {
    limit: z.number().optional(),
}, async ({ limit }) => {
    getDb();
...
// FILE HISTORY
// ═══════════════════════════════════════════════════
server.tool('cortex_get_file_history', 'Get complete history for a file: sessions, diffs, errors', {
    file_path: z.string(),
}, async ({ file_path }) => {
    getDb();
...
// HEALTH
// ═══════════════════════════════════════════════════
server.tool('cortex_get_health', 'Get project health score with metrics and trend', {}, async () => {
    getDb();
...
// UNFINISHED
// ═══════════════════════════════════════════════════
server.tool('cortex_get_unfinished', 'Get open/unresolved items — things that were started but not completed', {}, async () => {
    getDb();
...
// CONVENTIONS
// ═══════════════════════════════════════════════════
server.tool('cortex_get_conventions', 'List active conventions with violation counts', {
...
// ═══════════════════════════════════════════════════
server.tool('cortex_suggest_claude_md', 'Suggest CLAUDE.md updates based on new learnings and patterns', {}, async () => {
    getDb();
...
// ═══════════════════════════════════════════════════
server.tool('cortex_resolve_unfinished', 'Mark an unfinished item as resolved/done', { id: z.number(), session_id: z.string().optional() }, async ({ id, session_id }) => {
    getDb();
...
server.tool('cortex_get_stats', 'Get overall project statistics: sessions, decisions, errors, files, learnings', {}, async () => {
    const db = getDb();
...
server.tool('cortex_add_intent', 'Store a stated intention for follow-up in future sessions', {
    ...
    db.prepare(`INSERT INTO unfinished (session_id, created_at, description, context, priority) VALUES (?, ?, ?, 'intent', 'medium')`).run(session_id ?? null, ts, `[INTENT] ${intent}`);
...
server.tool('cortex_export', 'Export brain data as JSON or Markdown', { format: z.enum(['json', 'markdown']).optional().default('markdown') }, async ({ format }) => {
    const db = getDb();
    try {
        const data = {
            ...
            notes: (() => { try {
                return db.prepare(`SELECT text, tags, created_at FROM notes ORDER BY created_at DESC LIMIT 30`).all();
            }
            catch {
                return [];
            } })(),
        ...
        if (p?.name)
            md.push(`**${p.name}** · ${p.role ?? ''} · ${p.working_style ?? ''}`);
        md.push('');
...
// CROSS-PROJECT BRAIN
// ═══════════════════════════════════════════════════
server.tool('cortex_set_project', 'Set the active project name for context tagging', { project: z.string() }, async ({ project }) => {
    const db = getDb();
