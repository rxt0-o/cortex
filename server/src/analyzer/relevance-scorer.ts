...
export function scoreSession(
  ...
  if (recencyDays < 1) score += 30;
  else if (recencyDays < 3) score += 20;
  else if (recencyDays < 7) score += 10;
  ...
  if (sessionSummary) {
    for (const file of context.currentFiles) {
      ...
      if (sessionSummary.includes(basename)) score += 10;
    }
  }
...
export function selectTopItems(items: ScoredItem[], maxTokens: number): ScoredItem[] {
...
export function formatContextBlock(items: ScoredItem[]): string {
