import { getDb, type SQLInputValue } from '../db.js';
...
let surpriseCache: Map<string, number> | null = null;
let surpriseCacheSessionId: string | null = null;
...
export function computeImportance(params: {
  ...
  let impact = 0.5;
  if (impactSource === 'high' || impactSource === 'critical') impact = 1.0;
  else if (impactSource === 'medium') impact = 0.6;
  else if (impactSource === 'low') impact = 0.3;
 ...
 */
function getSurprise(entityType: string, sessionId?: string): number {
  ...
  for (const [type, table] of Object.entries(TABLE_MAP)) {
    try {
      const row = db.prepare(`SELECT COUNT(*) as c FROM ${table} WHERE archived_at IS NULL`).get() as { c: number };
      counts[type] = row.c;
      total += row.c;
    } catch {
      counts[type] = 0;
 ...
 */
export function refreshImportanceScores(table: string, entityType: string, sessionId?: string): number {
  const db = getDb();
  ...
  for (const row of rows) {
    const score = computeImportance({
      ...
      priority: row[priorityCol] as string | null,
      entityType,
      sessionId,
    });
    try {
      updateStmt.run(score as SQLInputValue, row.id as SQLInputValue);
      updated++;
    } catch {
