 ...
 */
const DECAY_TABLES: Array<{
  table: string;
  dateCol: string;
  extraImmunity?: string;
}> = [
 ...
 */
export function runDecay(maxTables = 5, startIndex = 0): number {
  const db = getDb();
 ...
 */
export function touchMemory(table: string, id: number): void {
  const db = getDb();
  try {
    db.prepare(`
      UPDATE ${table}
 ...
 */
export function getDecayStats(): Array<{ table: string; total: number; strong: number; weak: number; pinned: number }> {
  const db = getDb();
  const stats = [];

  for (const { table } of DECAY_TABLES) {
    try {
      const total = (db.prepare(`SELECT COUNT(*) as c FROM ${table} WHERE archived_at IS NULL`).get() as { c: number }).c;
      const strong = (db.prepare(`SELECT COUNT(*) as c FROM ${table} WHERE archived_at IS NULL AND memory_strength >= 0.1`).get() as { c: number }).c;
      const weak = (db.prepare(`SELECT COUNT(*) as c FROM ${table} WHERE archived_at IS NULL AND memory_strength IS NOT NULL AND memory_strength < 0.1`).get() as { c: number }).c;
      const pinned = (db.prepare(`SELECT COUNT(*) as c FROM ${table} WHERE memory_strength IS NULL`).get() as { c: number }).c;
