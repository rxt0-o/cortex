 ...
 */
export function spreadingActivation(seeds: Array<{ type: string; id: number }>): ActivatedNode[] {
  const activations = new Map<string, number>();
  const visited = new Set<string>();

  // Initialize seeds
  const queue: Array<{ type: string; id: number; activation: number; hop: number }> = [];
  ...
  for (const [key, activation] of activations) {
    ...
    if (activation < ACTIVATION_THRESHOLD) continue;
 ...
 */
export function activateForFiles(files: string[]): ActivatedNode[] {
  if (!files || files.length === 0) return [];

  const db = getDb();
  const seeds: Array<{ type: string; id: number }> = [];
  ...
  for (const file of files.slice(0, 5)) {
    ...
    try {
      ...
      for (const row of rows) seeds.push({ type: 'error', id: row.id });
    } catch { /* skip */ }
    ...
    try {
      ...
      `).all(`%${file}%`) as Array<{ id: number }>;
      for (const row of rows) seeds.push({ type: 'decision', id: row.id });
    } catch { /* skip */ }
  }
